...
# Start paxos node.
${PAXOS_NODE} -myport=${NODE_PORT0} -ports=${ALL_PORTS} -N=4 -id=0 -pxport=${PROXY_PORT} -proxy=0 -retries=10 2> /dev/null &
PAXOS_NODE_PID0=$!
sleep 1

${PAXOS_NODE} -myport=${NODE_PORT1} -ports=${ALL_PORTS} -N=4 -id=1 -pxport=${PROXY_PORT} -proxy=0 -retries=10 2> /dev/null &
PAXOS_NODE_PID1=$!
sleep 1

${PAXOS_NODE} -myport=${NODE_PORT2} -ports=${ALL_PORTS} -N=4 -id=2 -pxport=${PROXY_PORT} -proxy=0 -retries=10 2> /dev/null &
PAXOS_NODE_PID2=$!
sleep 1

${PAXOS_NODE} -myport=${NODE_PORT3} -ports=${ALL_PORTS} -N=4 -id=3 -pxport=${PROXY_PORT} -proxy=0 -retries=10 2> /dev/null &
PAXOS_NODE_PID3=$!
sleep 1

# Start paxostest.
${PAXOS_TEST} -port=${TESTER_PORT} -paxosports=${ALL_PORTS} -N=4 -nodeport=${NODE_PORT0} -pxport=${PROXY_PORT} -initwait=10 #-t=test
